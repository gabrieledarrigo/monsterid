/*
 * MonsterId for NodeJs, version 1.0.0
...
export default {
  async fillParts(avatar, size = 120, getRandomNumber) {
    const parts = {
    ...
    Object.keys(parts).forEach(async (part) => {
  ...
  async getAvatar(username = '') {
    ...
    const image = await gd.createTrueColorSync(DEFAULT_SIZE, DEFAULT_SIZE);
    const color = image.colorAllocate(255, 255, 255);
    image.fill(0, 0, color);

    return this.fillParts(image, DEFAULT_SIZE, getRandomNumber);
  },
};
